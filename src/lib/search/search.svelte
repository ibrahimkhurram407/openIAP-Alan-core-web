<script lang="ts">
  ...
  onMount(() => {
    setTimeout(() => {
      // Hack to remove focus from the button, since Dialog does not support overriding tabindex
      const button = Ref?.querySelector("button");
      if (button) {
        button.tabIndex = -1;
        const button2 = button?.querySelector("button");
        if (button2) {
          button2.tabIndex = -1;
        }
      }
    ...
    const unsubscribe2 = searchQuery.subscribe(async (value) => {
      ...
      const customers = await $client.Query<any>({
...
<div class={cn("search-container", className)} bind:this={Ref}>
  <Dialog.Root bind:open={hasFocus}>
    ...
    <Dialog.Content CloseButton={false}>
      ...
      {#each flatItems as item, index}
        ...
        <Dialog.Close
          class="text-left px-2 py-1 flex items-center text-sm item {index ===
          selectedIndex
            ? 'bg-muted/80'
            : ''}"
