  ...
  const GetData = async () => {
    try {
      ...
      if ($serverItemCount == -1) {
        $serverItemCount = await $client.Count({
          collectionname,
          query: $currentquery,
        });
        if ($pageIndex > 0) {
...
<div class={cn("", className)} use:swipe={{ timeframe: 300, minSwipeDistance: 100}} on:swipe={handlerswipe}>
            ...
            <Table.Row>
              {#each headerRow.cells as cell (cell.id)}
                ...
                >
                  ...
                  {:else if fixedColumnsSizes[cell.id] != null && cell.id != "id" && cell.id != "_id"}
                    <Table.Head
                      {...attrs}
            ...
            <Table.Row {...rowAttrs}>
              {#each row.cells as cell (cell.id)}
                <Subscribe attrs={cell.attrs()} let:attrs>
                  ...
                  {:else if cell.id === "name" && multiselect == true}
                    <Table.Cell
                      {...attrs}
                      on:click={() => {
                        // @ts-ignore
  ...
  <div class="flex items-center space-x-4 py-4">
    ...
    <HotkeyButton
      ...
      on:click={() => dispatch("insert", { collectionname: collectionname })}
      >Insert</HotkeyButton
    >
    <HotkeyButton
      variant="outline"
    ...
    >
    <HotkeyButton hidden data-shortcut={"Alt+r,Meta+r"} on:click={GetData}
      >Reload</HotkeyButton
    >
    <HotkeyButton
      hidden
