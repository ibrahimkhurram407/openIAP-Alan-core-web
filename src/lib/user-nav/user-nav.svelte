...
{#if $isAuthenticated == true}
  <HotkeyButton
    hidden
    data-shortcut={"Control+p,Meta+p"}
    on:click={() => goto(base + `/users/${$user.profile.sub}`)}
    >Profile</HotkeyButton
  >
  <HotkeyButton
    hidden
    data-shortcut={"Control+b,Meta+b"}
    on:click={() => goto(base + `/customers/billing`)}>Billing</HotkeyButton
  >
  <HotkeyButton
    hidden
    data-shortcut={"Control+s,Meta+s"}
    on:click={() => goto(base + `/users/settings`)}>Settings</HotkeyButton
  >
  <DropdownMenu.Root>
