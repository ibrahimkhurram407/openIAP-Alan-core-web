...
export default {
  ...
  actions: {
    async GetConfig({ commit }) {
      ...
      var res2 = await fetch((process.env.NODE_ENV === 'production' ? '/config' : data.VUE_APP_CONFIG_URL));
    ...
    async Signin({ commit, dispatch, getters }) {
      try {
        ...
        if (client == null) {
          // console.debug(`Connecting to ${domain}`)
          if(!wsurl.endsWith("/")) wsurl = wsurl + "/";
          if(wsurl.indexOf("ws/v2") == -1) wsurl = wsurl + "ws/v2";
          
          var client = new openiap(wsurl, accessToken) // wsurl
          ...
          if (result && result.user) {
            // console.debug(`Signed in as ${result.user.name}`)
